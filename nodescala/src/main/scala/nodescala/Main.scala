...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val myServerSubscription = myServer.start("/test"){ request => for (kv <- request.iterator) yield kv + "\n" }

    // TO IMPLEMENT
    // 2. create a future that expects some user input `x`
    //    and continues with a `"You entered... " + x` message
    val userInterrupted: Future[String] = Future.userInput("Press return.").continueWith(_ => "You entered... ")
